            ...
            loop {
                futures::select! {
                    message = write_reqs_rx.select_next_some() => {
                        ...
                        let is_err = if outbound_stream.should_stream(&message) {
                            outbound_stream.stream_message(message).await.is_err()
                        } else {
                            msg_tx.send(MultiplexMessage::Message(message)).await.is_err()
                        };
                        if is_err {
                            break;
                        }
