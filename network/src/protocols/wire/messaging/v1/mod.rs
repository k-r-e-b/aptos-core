...
//!
//! The [AptosNet specification](https://github.com/aptos-labs/aptos-core/blob/main/specifications/network/messaging-v1.md)
...
impl NetworkMessage {
    pub fn len(&self) -> usize {
        match self {
            NetworkMessage::Error(_) => 2, // enum tag + u8
            NetworkMessage::RpcRequest(request) => 6 + request.raw_request.len(), // headers are 6 bytes
            NetworkMessage::RpcResponse(response) => 5 + response.raw_response.len(), // headers are 5 bytes
            NetworkMessage::DirectSendMsg(message) => 2 + message.raw_msg.len(), // headers are 2 bytes
        }
    }
}
