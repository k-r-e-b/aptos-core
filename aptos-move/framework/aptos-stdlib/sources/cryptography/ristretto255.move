...
module aptos_std::ristretto255 {
    ...
    public fun multi_scalar_mul(points: vector<RistrettoPoint>, scalars: vector<Scalar>): RistrettoPoint {
        // TODO: ugh... these get moved in, we could return them back to the caller? or take a reference to the vector!
        // not sure how to handle that in move
        points;
        scalars;
    ...
    public fun new_scalar_from_64_uniform_bytes(bytes: vector<u8>): Option<Scalar> {
        if (std::vector::length(&bytes) == 64) {
            std::option::some(Scalar {
                data: scalar_from_64_uniform_bytes_internal(bytes)
            })
        } else {
            std::option::none()
        }
    ...
    native fun scalar_from_64_uniform_bytes_internal(bytes: vector<u8>): vector<u8>;
    ...
    // The scalar 2
    const TWO_SCALAR: vector<u8> = vector[
    ...
    const X_SCALAR: vector<u8> = vector[
    ...
    const X_INV_SCALAR: vector<u8> = vector[
    ...
    const X_INV_SCALAR_BE_BITS: vector<bool> = vector[
    ...
    const X_INV_SCALAR_LE_BITS: vector<bool> = vector[
    ...
    const Y_SCALAR: vector<u8> = vector[
    ...
    const X_TIMES_Y_SCALAR: vector<u8> = vector[
    ...
    // X + 2^256 * X \mod \ell
    const CANONICAL_X_PLUS_2_TO_256_TIMES_X_SCALAR : vector<u8> = vector[
    ...
    const CANONICAL_2_256_MINUS_1_SCALAR: vector<u8> = vector[
    ...
    // ```
    const B_SCALAR: vector<u8> = vector[
        0xdb, 0xfd, 0x97, 0xaf, 0xd3, 0x8a, 0x06, 0xf0,
        0x13, 0x8d, 0x05, 0x27, 0xef, 0xb2, 0x8e, 0xad,
        0x5b, 0x71, 0x09, 0xb4, 0x86, 0x46, 0x59, 0x13,
        0xbf, 0x3a, 0xa4, 0x72, 0xa8, 0xed, 0x4e, 0x0d,
    ];

    const A_TIMES_B_SCALAR: vector<u8> = vector[
        0x2a, 0xb5, 0x0e, 0x38, 0x3d, 0x7c, 0x21, 0x0f,
        0x74, 0xd5, 0x38, 0x73, 0x30, 0x73, 0x5f, 0x18,
        0x31, 0x51, 0x12, 0xd1, 0x0d, 0xfb, 0x98, 0xfc,
        0xce, 0x1e, 0x26, 0x20, 0xc0, 0xc0, 0x14, 0x02,
    ];

    const A_PLUS_B_SCALAR: vector<u8> = vector[
        0x08, 0x38, 0x39, 0xdd, 0x49, 0x1e, 0x57, 0xc5,
        0x74, 0x37, 0x10, 0xc3, 0x9a, 0x91, 0xd6, 0xe5,
        0x02, 0xca, 0xb3, 0xcf, 0x0e, 0x27, 0x9a, 0xe4,
        0x17, 0xd9, 0x1f, 0xf2, 0xcb, 0x63, 0x3e, 0x07,
    ];
    ...
    /// ```
    const A_TIMES_BASE_POINT: vector<u8> = vector[
        0x96, 0xd5, 0x2d, 0x92, 0x62, 0xee, 0x1e, 0x1a,
        0xae, 0x79, 0xfb, 0xae, 0xe8, 0xc1, 0xd9, 0x06,
        0x8b, 0x0d, 0x01, 0xbf, 0x9a, 0x45, 0x79, 0xe6,
        0x18, 0x09, 0x0c, 0x3d, 0x10, 0x88, 0xae, 0x10,
    ];
...
//    ];
    ...
    fun test_scalar_from_256_bits() {
        ...
        assert!(scalar_equals(&biggest, &Scalar { data: CANONICAL_2_256_MINUS_1_SCALAR }), 1);
    }
    ...
    fun test_scalar_from_512_bits() {
        ...
        let reduced = std::option::extract(&mut new_scalar_from_64_uniform_bytes(x_plus_2_to_256_times_x));
        let expected = Scalar { data: CANONICAL_X_PLUS_2_TO_256_TIMES_X_SCALAR };
        assert!(scalar_equals(&reduced, &expected), 1)
    }

    #[test]
    fun test_scalar_little_endian_bits() {
        let xinv = Scalar { data: X_INV_SCALAR };
        ...
        assert_same_bits(len, bits, X_INV_SCALAR_LE_BITS);
        ...
        assert_same_bits(len, bits, X_INV_SCALAR_BE_BITS);
    }
