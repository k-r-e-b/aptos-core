...
pub trait SubstreamProcessor: Send + Sync + Debug {
    ...
    /// Accepts a transaction, and processes it. This method will be called from `process_substream_with_status`
    /// In case a transaction cannot be processed, returns an error: the processor will mark it as failed in the database,
