    ...
    def run(self, context: ForgeContext) -> ForgeResult:
        ...
        with ForgeResult.with_context(context) as forge_result:
            result = context.shell.run([
                ...
                "test", "k8s-swarm",
                *(["--image-tag", context.forge_image_tag] if context.forge_image_tag else []),
...
def test(
    ...
    # Pre flight checks
        ...
        try:
            ...
            assert_aws_token_expiration(aws_token_expiration)
        except AwsError:
    ...
    if forge_pre_comment:
        outputs.append(ForgeFormatter(forge_pre_comment, lambda *_: pre_comment))
    if forge_comment:
